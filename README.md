...
| `async-10-deeplink-go-router` | 07. Навигация и состояние | `go_router`, вложенные маршруты, схема `yamovies://` |
...
## async-10-deeplink-go-router

Демо «Диплинк». Экран перестаёт быть местом, куда толкают `Navigator.push`,
и становится адресом, из которого выводится стек.

> [!NOTE]
> В лекции сквозной пример называется MovieBox и схема на слайде —
> `moviebox://`. В репозитории приложение называется YaMovies с первой лекции
> курса, поэтому схема здесь `yamovies://`.

**Где смотреть в приложении**

Экран фильма → секция «Cast» → кнопка **«All cast»** (тот же адрес, что
у диплинка) и строка с готовой командой `adb` внизу экрана. Снаружи —
`./demo.sh link`, а несуществующий адрес показывает экран ошибки роутера
с рабочим примером ссылки.

**Файлы**

| Файл | Что в нём |
|---|---|
| `lib/application/router/app_router.dart` | Дерево маршрутов и константы путей. Вложенность и есть маппинг ссылки в стек: `/movie/278/cast` разворачивается в три страницы. Здесь же `normalizeDeepLink` — без неё `yamovies://movie/278/cast` не находит маршрут: система считает `movie` хостом, и до роутера доезжает путь `/278/cast` |
| `lib/application/movie_app.dart` | `MaterialApp.router`; роутер создаётся один раз в `State` |
| `lib/application/module/details/movie_cast_screen.dart` | Экран «Актёры» на `FutureBuilder` — цель диплинка. Future создаётся в `initState`, а не в `build` |
| `lib/application/module/details/_cast_row.dart` | Кнопка «All cast» ведёт по тому же адресу, что и ссылка |
| `lib/application/module/details/_details_demo_panel.dart` | Готовая команда `adb shell am start …` с id открытого фильма |
| `lib/application/module/list/movie_list_controller.dart`, `_movie_list_view.dart`, `_search_results.dart` | Навигация переехала с `Navigator.push` на `context.go(...)` |
| `android/app/src/main/AndroidManifest.xml` | `intent-filter` со схемой `yamovies` и `flutter_deeplinking_enabled` — без флага движок не отдаст ссылку роутеру |
| `ios/Runner/Info.plist` | `CFBundleURLTypes` и `FlutterDeepLinkingEnabled` |
| `demo.sh` | Команда `link` отправляет ссылку через `adb` или `simctl`, `install` ставит APK там, где `flutter run` не видит устройство |
| `test/app_router_test.dart` | Вложенность маршрутов, нормализация обеих форм ссылки, состав демо-экранов |

> [!IMPORTANT]
> В ссылке `yamovies://movie/278/cast` первый сегмент система считает **хостом**,
> и до роутера доезжает путь `/278/cast` — такого маршрута нет, экран показал бы
> ошибку. Поэтому в `createAppRouter` стоит `redirect` с `normalizeDeepLink`:
> он склеивает хост обратно с путём. Форма с тремя слэшами
> (`yamovies:///movie/278/cast`) разбирается сама и проходит мимо нормализации.

**Как показывать**

1. `./demo.sh link` (или `adb shell am start -a android.intent.action.VIEW
   -d "yamovies://movie/278/cast"`) — открывается вкладка «Актёры».
2. Кнопка «назад» ведёт на экран фильма, оттуда — на ленту: стек восстановлен
   целиком.
3. Закрыть приложение полностью и повторить ту же ссылку — результат совпадает.
   На iOS: `xcrun simctl openurl booted "yamovies://movie/278/cast"`.

## Ревью архитектуры: что поправлено во всех примерах

Отдельный проход по всем веткам — на слои, ресурсы и практики Dart/Flutter.
Правки лежат в тех ветках, где появляется соответствующий код.

**Найденные дефекты**

- `MovieSearchController` звал `notifyListeners` после `dispose` (ветка 05):
  уход с экрана поиска посреди запроса ронял `ChangeNotifier`. Нашлось тестом
  «закрытие контроллера посреди запроса». Заодно запрос переехал
  на `Stream.multi`: поток обязан завершиться, иначе `switchMap` не подпишется
  на следующий и строка поиска молча замирает на первом ответе.
- `RefreshInterceptor` не разлогинивал при повторном 401 (ветка 06): повтор
  уходит через «голый» клиент, у которого нет этого интерсептора, поэтому
  второй 401 в него не возвращался. Теперь проверяется в ветке ошибки повтора.
- Сетевой слой нельзя было протестировать: клиент повторов создавался внутри
  и в тестах не подменялся. У фабрики появился параметр `retryDio` — и семь
  тестов на цепочку интерсепторов.

**Практики**

- `equatable` для доменных моделей, состояний и событий BLoC (ветка 02):
  `BlocBuilder` больше не перерисовывает экран на одинаковых данных.
- `emit.isDone` после каждого ожидания в блоках — как предписывает bloc.
- `@immutable` снят с `DependencyContainer`: он держит объекты с состоянием.
  Вместо аннотации у контейнера есть `dispose`, а вызывает его `DependencyOwner` —
  виджет-владелец. Он же закрывает базу, сервисы и шину событий.
- Подписки на чужие `ChangeNotifier` снимаются: `SwitchableTokenStorage`
  и `SyncService.listenToConnectivity` (ветки 07 и 09).
- Строки таблиц drift больше не уезжают в UI: экран кэша ходит через
  репозиторий (`CacheSnapshot`), панель очереди получает `OutboxEntry`.
  Слайд лекции требует конвертировать модели на границе репозитория —
  теперь примеры этому и следуют.
- `Movie.fromJson` не притворяется, что фильм без `id` бывает: остальные поля
  получают значения по умолчанию, а идентификатор падает осознанно.

**Тесты**

От 11 в первой ветке до 37 в последней: блок экрана фильма (сумма против
максимума, уход с экрана посреди загрузки), контроллер поиска на `fake_async`
и в реальном времени, цепочка интерсепторов на подменённом `HttpClientAdapter`,
кэш и очередь на `NativeDatabase.memory()`, вложенность маршрутов.

> [!NOTE]
> Тест `switchMap` идёт в реальном времени, а не в `fake_async`: связка rxdart
> с фиктивными часами не доводит до конца внутренние потоки, и проверка
> получилась бы про поведение библиотеки в тесте, а не про наш код.
